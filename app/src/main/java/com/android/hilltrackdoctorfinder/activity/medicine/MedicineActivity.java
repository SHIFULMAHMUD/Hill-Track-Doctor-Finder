...
import com.android.hilltrackdoctorfinder.adapter.MedicinePagerAdapter;
...
public class MedicineActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void setupViewPager() {
        MedicinePagerAdapter adapter = new MedicinePagerAdapter(getSupportFragmentManager());
        adapter.addFragment(new NearestPharmacyFragment()); //index 0
        adapter.addFragment(new MedicineShopFragment()); //index 1
        viewPager.setAdapter(adapter);
        tabLayout.setupWithViewPager(viewPager);
        tabLayout.setTabGravity(tabLayout.GRAVITY_FILL);
        tabLayout.getTabAt(0).setText(R.string.nearest_pharmacy);
        tabLayout.getTabAt(1).setText(R.string.buy_online);
    }
